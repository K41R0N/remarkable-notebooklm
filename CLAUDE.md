...
- Do not assign `Image.open()` without an explicit type annotation under mypy strict — use `image: Image.Image = Image.open(...)` instead of bare assignment; `Image.open()` returns `ImageFile` (a subtype of `Image.Image`), and subsequent reassignments will fail with `Incompatible types in assignment` (see `docs/solutions/build-errors/pil-image-open-strict-type-annotation.md`)
- Do not compare PIL image pipeline test ratios against original input dimensions — the pipeline crops before resizing, so the crop step changes the aspect ratio; always compare against post-crop dimensions as the baseline (see `docs/solutions/test-failures/image-crop-aspect-ratio-baseline-mismatch.md`)
